...
export default function RegisterPage() {
  ...
  function validateFormInline() {
    ...
    if (username.trim().length < 2) errs.username = 'Tên dang nh?p ph?i có ít nh?t 2 ký t?.';
    if (!reEmail.test(email)) errs.email = 'Email không h?p l?.';
    if (!strongPw.test(password)) errs.password = 'M?t kh?u ph?i = 8 ký t?, g?m hoa, thu?ng, s? và ký t? d?c bi?t.';
    if (password !== password2) errs.password2 = 'M?t kh?u xác nh?n không kh?p.';
    if (!agreed) errs.agreed = 'B?n ph?i d?ng ý v?i Ði?u kho?n và Chính sách.';
    return errs;
  }
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      setToast({ type: 'success', message: 'OTP m?i dã du?c g?i!' });
    } catch (err) {
  ...
  async function handleOtpSubmit(e) {
    e.preventDefault();
    if (!/^\d{6}$/.test(otp)) {
      setOtpError('OTP ph?i là 6 ch? s?');
      return;
    }
    setOtpLoading(true);
    setOtpError("");
    try {
      ...
      setToast({ type: 'success', message: 'Ðang ký thành công!' });
  ...
  async function handleResendOtp() {
    ...
    try {
      ...
      setOtpError('G?i l?i OTP th?t b?i. Vui lòng th? l?i.');
    }
    setOtpLoading(false);
  }
  ...
  const RegisterForm = (
    <form onSubmit={handleSubmit} className="flex flex-col gap-4 w-full max-w-[360px] mx-auto px-2 py-10" aria-label="form dang ký">
      <h1 className="font-extrabold text-4xl text-blue-600 text-center mb-1">ÐANG KÝ</h1>
      <div className="text-center text-gray-600 mb-4 text-base font-semibold uppercase tracking-wide">CHÀO M?NG!</div>

      <label className="text-xs text-gray-700 font-semibold" htmlFor="username">Tên dang nh?p</label>
      <input
        id="username"
        type="text"
      ...
      <label className="text-xs text-gray-700 font-semibold" htmlFor="password2">Xác nh?n m?t kh?u</label>
      <div className="relative">
        <input
          id="password2"
      ...
      <label className="flex items-center gap-2 mt-2 text-gray-800 text-sm cursor-pointer select-none">
        ...
        <span>
          Tôi d?ng ý v?i{' '}
          <a href="/terms" target="_blank" className="text-blue-600 underline hover:opacity-80">Ði?u kho?n s? d?ng</a>{' '}và{' '}
          <a href="/privacy" target="_blank" className="text-blue-600 underline hover:opacity-80">Chính sách b?o m?t</a>
        </span>
      </label>
      ...
      <div className="text-sm text-center text-gray-500 mt-0">
        B?n dã có tài kho?n?{' '}
        <a href="/login" className="text-blue-600 font-semibold hover:underline">Ðang nh?p</a>
      </div>
  ...
  const OtpForm = (
    <form onSubmit={handleOtpSubmit} className="flex flex-col gap-5 w-full max-w-[370px] mx-auto px-2 animate-fade-in" aria-label="form xác minh OTP">
      ...
      <div className="text-center text-gray-600 mb-2 text-base font-medium">
        Mã xác th?c dã g?i t?i email:{' '}<span className="text-blue-700 font-semibold break-all">{email}</span>
      </div>
      <input
        type="text"
        inputMode="numeric"
